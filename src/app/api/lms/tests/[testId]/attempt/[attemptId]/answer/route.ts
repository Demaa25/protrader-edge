// src/app/api/lms/tests/[testId]/attempt/[attemptId]/answer/route.ts
  ...
  try {
    ...
    const attempt = await prisma.evaluationAttempt.findUnique({
      ...
      select: {
        ...
        evaluationId: true,
        startedAt: true,
        evaluation: { select: { passMarkPct: true } },
      },
    });
    ...
    if (attempt.status === AttemptStatus.SUBMITTED || elapsed > TEST_DURATION_MS) {
      return NextResponse.json({ error: "Time elapsed. Test already submitted." }, { status: 400 });
    }
